...
Run the [Cut Release workflow](../../actions/workflows/cut-release.yaml) (or `gh workflow run cut-release.yaml`), picking a `patch`/`minor`/`major` bump. Everything else is automatic:

1. **Cut Release** computes the next `vX.Y.Z` tag and publishes a GitHub release from `master`.
2. **Release** builds the frontend and attaches `lab-<tag>.tar.gz` / `lab-<tag>.zip` to the release.
...
Publishing a release by hand through the GitHub UI also works — step 2 onwards is triggered by the release publish event, not by the Cut Release workflow.
