...
// Activities holds implementations for all timings activities
...
func (a *Activities) processBlockTimings(ctx context.Context, network string, timeRange struct{ Start, End time.Time }) (*TimingData, error) {
	...
	for _, row := range rows {
		...
		if _, exists := result.Validators[proposerIndex]; !exists {
			result.Validators[proposerIndex] = make(map[string]int)
		}
		...
		for metric, value := range row {
			if metric != "proposer_index" {
				...
				result.Validators[proposerIndex][metric] = int(intValue)
			}
		}
	}
...
func (a *Activities) processSizeCDF(ctx context.Context, network string, timeRange struct{ Start, End time.Time }) (*SizeCDFData, error) {
	...
	result := &SizeCDFData{
		...
		TimeStamp: time.Now().UTC(),
		SizesKB:   sizesKB,
		ArrivalTimesMs: map[string][]float64{
			"all":          allArrivalTimes,
			"mev":          mevArrivalTimes,
			"non_mev":      nonMevArrivalTimes,
			"solo_mev":     soloMevArrivalTimes,
			"solo_non_mev": soloNonMevArrivalTimes,
		},
