...
const BLOCK_TYPE_DESCRIPTIONS = {
  ...
  'MEV Blocks':
    'Blocks that were built by MEV-Boost relays, which may have different arrival characteristics due to their specialized construction.',
...
export const BlockTimings: React.FC = () => {
  ...
  const timeWindows = useMemo<TimeWindowConfig[]>(() => {
    const moduleConfig = config?.modules?.['beaconChainTimings'];
  ...
  const defaultNetwork = useMemo(() => {
    const moduleConfig = config?.modules?.['beaconChainTimings'];
    return moduleConfig?.networks &&
      Array.isArray(moduleConfig.networks) &&
      moduleConfig.networks.length > 0
  ...
  const timingsPath = config?.modules?.['beaconChainTimings']?.pathPrefix
    ? `${config.modules['beaconChainTimings'].pathPrefix}/block_timings/${network}/${timeWindow}`
    : null;

  const cdfPath = config?.modules?.['beaconChainTimings']?.pathPrefix
    ? `${config.modules['beaconChainTimings'].pathPrefix}/size_cdf/${network}/${timeWindow}`
    : null;

  const { data: timingData, error } = useDataFetch<TimingData>(baseUrl, timingsPath);
  ...
  const formatTime = useMemo(
    () => (time: number) => {
      ...
      return currentWindow?.step === '15m' ? date.toLocaleTimeString() : date.toLocaleDateString();
    },
    [currentWindow],
  );
  ...
  const scatterData = useMemo(() => {
    ...
    const allData = (cdfData.arrival_times_ms.all?.values || [])
    ...
    const mevData = (cdfData.arrival_times_ms.mev?.values || [])
      .map((time: number, index: number) => ({
        arrival_time: time / 1000,
        size: cdfData.sizes_kb[index],
        type: 'mev',
    ...
    const nonMevData = (cdfData.arrival_times_ms.non_mev?.values || [])
      .map((time: number, index: number) => ({
        arrival_time: time / 1000,
        size: cdfData.sizes_kb[index],
        type: 'non_mev',
    ...
    const soloMevData = (cdfData.arrival_times_ms.solo_mev?.values || [])
      .map((time: number, index: number) => ({
        arrival_time: time / 1000,
        size: cdfData.sizes_kb[index],
        type: 'solo_mev',
    ...
    const soloNonMevData = (cdfData.arrival_times_ms.solo_non_mev?.values || [])
      .map((time: number, index: number) => ({
        arrival_time: time / 1000,
        size: cdfData.sizes_kb[index],
        type: 'solo_non_mev',
    ...
    return [...allData, ...mevData, ...nonMevData, ...soloMevData, ...soloNonMevData].sort(
      (a, b) => a.size - b.size,
    );
  }, [cdfData]);
  ...
  const nivoChartData = useMemo(() => {
    ...
    return lineNames
      ...
      .filter(series => {
        const key = Object.entries(PERCENTILE_LABELS).find(
          ([, label]) => label === series.id,
        )?.[0] as PercentileKey;
  ...
  return (
    <div className="space-y-6">
      <AboutThisData>
        <p>
          View block propagation timing metrics across the network. This data helps us understand
          how quickly blocks are propagating through the network and identify potential bottlenecks.
        </p>
      </AboutThisData>
      ...
      <section className="space-y-12">
        ...
        <div className="space-y-6">
          <div>
            <h2 className="text-2xl font-sans font-bold text-primary">
              Block Arrival Time Distribution
            </h2>
            <p className="text-sm font-mono text-tertiary mt-1">
              This chart tracks block arrival times across the network, showing how quickly blocks
              propagate to nodes. The data is collected by nodes run by the ethPandaOps team and
              contributed by the community, showing aggregated results.
            </p>
          </div>
              ...
              tooltip={({ point }) => {
                const xValue =
                  typeof point.data.x === 'number'
                    ? point.data.x
                    : parseFloat(point.data.x as string);
                const yValue =
                  typeof point.data.y === 'number'
                    ? point.data.y
                    : parseFloat(point.data.y as string);
              ...
              legends={[
                {
                  ...
                  onClick: (data, event) => {
                    ...
                    if (key) {
                      handleLegendClick(event as unknown as React.MouseEvent<HTMLButtonElement>, {
                        value: key,
              ...
              <tbody>
                {getAvailableLines('arrival').map(line => {
                  ...
                  const values = chartData
                    .map(d => d[dataKeyMap[line as PercentileKey] as keyof typeof d])
                    ...
                    <tr key={line} className="border-b border-default hover:bg-hover">
                      <td className="py-2 px-3 flex items-center gap-2">
                        <div
                          ...
                          style={{ backgroundColor: PERCENTILE_COLORS[line as PercentileKey] }}
                        />
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-sm font-mono text-tertiary mt-1">
              This chart analyzes the relationship between block size and network propagation time,
              helping identify how block size impacts network performance. The data is collected by
              nodes run by the ethPandaOps team and contributed by the community, showing aggregated
              results.
            </p>
          </div>
          ...
          <div className="h-[500px] w-full" style={{ cursor: 'pointer' }}>
            <NivoLineChart
              ...
              colors={({ id }) =>
                BLOCK_TYPE_COLORS[id as keyof typeof BLOCK_TYPE_COLORS] || '#00ff9f'
              }
              lineWidth={2}
              pointSize={0}
              enableSlices="x"
