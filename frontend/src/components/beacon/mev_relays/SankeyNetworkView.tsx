...
const FlowStage: React.FC<{
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex-1 ${stageType === 'node' ? 'p-1' : 'p-1.5'} rounded-lg overflow-y-auto transition-colors duration-500 ${isActive ? 'bg-surface/30' : 'bg-surface/20'}`}
      >
      ...
      <div className="h-4 w-20 relative">
        <div
          className={`absolute top-1/2 -translate-y-1/2 h-[1px] w-full transition-colors duration-300 ${
            !isActive ? 'bg-tertiary/20 opacity-50' : isWinning ? 'bg-gold/70' : 'bg-tertiary/30'
          }`}
        />
...
const countUniqueBuilderPubkeys = (
...
export const SankeyNetworkView: React.FC<BlockFlowViewProps> = ({
        ...
        blobGasUsed:
          typeof block.execution_payload_blob_gas_used !== 'undefined'
            ...
            : typeof block.executionPayloadBlobGasUsed !== 'undefined'
              ...
              : 'undefined',
        totalBytes:
          typeof block.block_total_bytes !== 'undefined'
    ...
    timeFilteredBids.forEach(bid => {
      ...
      if (!uniqueRelays.has(relayName) || bid.isWinning) {
        uniqueRelays.set(relayName, {
          ...
          color:
            winningBid?.relayName === relayName ? '#FFD700' : relayColors[relayName] || '#3498db',
  ...
  const continents = useMemo(() => {
    ...
    if (blockTime !== undefined) {
      const hasAnyRealNodeData =
        Object.keys(nodeBlockSeen).length > 0 || Object.keys(nodeBlockP2P).length > 0;
      ...
      if (!hasAnyRealNodeData) {
        Object.values(continentsMap).forEach(continentData => {
          ...
          const hasRealTiming = continentData.nodeTimings.some(
            timing => timing.firstSeenTime !== undefined,
          );
      ...
      <div className="bg-surface/30 py-3 px-3 rounded-lg">
        ...
        <div className="border-b border-subtle/30">
          <div className="flex justify-between items-center pt-3 px-4">
            ...
            <div className="font-mono text-xs text-tertiary/80">
          ...
          <div className="relative px-4 pt-2 pb-6">
            ...
            <div className="h-2 mb-3 flex rounded-lg overflow-hidden border border-subtle relative">
              ...
              {(() => {
                ...
                const isInPropagation = isInPropagationPhase(
                  currentTime,
                  nodeBlockSeen,
                  nodeBlockP2P,
                );
            ...
            <div className="flex justify-between items-center px-6 relative z-10">
              {/* Builders Phase */}
              <div
                className={`flex flex-col items-center text-center ${!isActive('builder') ? 'opacity-60' : ''}`}
              >
                <div
              ...
              <div className="flex-shrink-0 flex items-center justify-center relative">
                <div
                  className={`h-0.5 w-[40px] ${isActive('relay') ? (isActive('builder') ? 'bg-active' : 'bg-active/40') : 'bg-tertiary/20'} transition-colors duration-500`}
                ></div>
              </div>

              {/* Relays Phase */}
              <div
                className={`flex flex-col items-center text-center ${!isActive('relay') ? 'opacity-60' : ''}`}
              >
                <div
                  className={`w-16 h-16 flex items-center justify-center rounded-full mb-2 shadow-md transition-colors duration-500 ${
                    isActive('relay')
              ...
              <div className="flex-shrink-0 flex items-center justify-center relative">
                <div
                  className={`h-0.5 w-[40px] ${isActive('proposer') ? (isActive('relay') ? 'bg-active' : 'bg-active/40') : 'bg-tertiary/20'} transition-colors duration-500`}
                ></div>
              </div>

              {/* Proposer Phase */}
              <div
                className={`flex flex-col items-center text-center ${!isActive('proposer') ? 'opacity-60' : ''}`}
              >
                <div
                  className={`w-16 h-16 flex items-center justify-center rounded-full mb-2 shadow-md transition-colors duration-500 ${
                    isActive('proposer')
              ...
              <div className="flex-shrink-0 flex items-center justify-center relative">
                <div
                  className={`h-0.5 w-[40px] ${isActive('node') ? (isActive('proposer') ? 'bg-active' : 'bg-active/40') : 'bg-tertiary/20'} transition-colors duration-500`}
                ></div>
              </div>

              {/* Network Nodes Phase */}
              <div
                className={`flex flex-col items-center text-center ${!isActive('node') ? 'opacity-60' : ''}`}
              >
                <div
                  className={`w-16 h-16 flex items-center justify-center rounded-full mb-2 shadow-md transition-colors duration-500 ${
                    isActive('node')
        ...
        <div className="grid grid-cols-7 h-[420px] gap-0">
          ...
          <div className="col-span-1">
            <div
              className={`flex-1 flex items-center justify-center pt-2 pb-16 ${isActive('proposer') ? 'opacity-100 bg-surface/30' : 'opacity-40 bg-surface/20'} transition-opacity transition-colors duration-700`}
            >
                      ...
                      return (
                        <BeaconBlockVisualization
                          ...
                          execution_block_hash={
                            block?.executionPayloadBlockHash || block?.execution_payload_block_hash
                          }
        ...
        <div className="text-xs text-tertiary mt-2 px-1">
          <p>
            This visualization demonstrates the complete Ethereum block production process: Builders
            proposing blocks, Relays forwarding bids, Proposers selecting blocks, and Network nodes
            receiving blocks.
          </p>
        </div>
      </div>
