  ...
  const bids = useMemo(() => {
    ...
    Object.entries(slotData.relayBids).forEach(([relay, relayBids]) => {
      relayBids.bids.forEach(bid => {
        if (bid.slotTime <= currentTime) {
          // Only show bids up to current time
          result.push({
            relay,
    ...
    <div className="relative h-80 overflow-hidden bg-gray-900/70 rounded-xl border border-gray-800 p-2">
          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* Value display */}
                <div className="font-bold text-white text-sm">{bid.value.toFixed(4)}</div>
      ...
      <div className="absolute bottom-1 right-1 bg-gray-900/80 p-2 rounded-md border border-gray-800">
        <div className="text-xs text-gray-400">
          ...
          {bids.length > 0 && (
            <div className="text-green-400">Highest: {highestBid.value.toFixed(4)} ETH</div>
          )}
        </div>
      </div>
    </div>
