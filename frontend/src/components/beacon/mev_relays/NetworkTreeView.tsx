...
export const NetworkTreeView: React.FC<NetworkTreeViewProps> = ({
  ...
  const isActive = (role: 'proposer' | 'relay' | 'builder' | 'node', time?: number) => {
    ...
    if (bids.length > 0) {
      ...
      if (role === 'proposer' && winningBid) {
        ...
        const winningBidTime = bids.find(bid => 
          bid.blockHash === winningBid.blockHash && 
          bid.relayName === winningBid.relayName
        )?.time;
  ...
  const networkData = useMemo(() => {
    ...
    Object.entries(relaysMap).forEach(([relayName, relayBids]) => {
      ...
      topBuilders.forEach(builder => {
        if (relayBuilders.has(builder.key) && isRelayActive) {
          ...
          const hasWinningBid = relayBids.some(bid => 
            bid.isWinning && 
            bid.builderPubkey === builder.key && 
            currentTime >= bid.time
          );
    ...
    return { nodes, links };
  }, [bids, relayColors, relaysMap, topBuilders, proposer, winningBid, nodesByContinent, currentTime]);

  return (
    <Card className="mb-4">
      <CardBody>
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-sans font-bold text-primary">Ethereum Block Production Flow</h3>
          {winningBid && (
        ...
        <div className="text-xs text-tertiary mt-2">
          <p>This diagram shows the flow of block production in Ethereum. Components fade in as they become active in the slot.</p>
        </div>
      </CardBody>
    </Card>
  );
