...
export const MultiSlotEventTimeline: React.FC<MultiSlotEventTimelineProps> = ({
  ...
  const processedEvents = useMemo((): TimelineEvent[] => {
    ...
    const processSlot = (slotData: BeaconSlotData | null | undefined, slotOffset: number) => {
      ...
      if (slotData.deliveredPayloads) {
        Object.entries(slotData.deliveredPayloads).forEach(
          ([relay, deliveredPayloadsContainer]) => {
  ...
  const payloadCount = processedEvents.filter(
    e => e.type === 'payload' && e.time <= currentTime,
  ).length;
  const blockCount = processedEvents.filter(
    e => e.type === 'block' && e.time <= currentTime,
  ).length;
