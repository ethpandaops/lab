...
const useFilteredBlockEvents = (blockEvents: BlockEvent[]) => {
  return useMemo(
    () =>
      blockEvents.filter(
        event => event.type === 'block_seen' && (event.source === 'api' || event.source === 'p2p'),
      ),
    [blockEvents],
  );
};
...
const useMarkers = (
  ...
  filteredBlockEvents: BlockEvent[],
  currentTime: number,
) => {
  return useMemo(() => {
  ...
  const handlePointerEnter = useCallback(
    (_: React.PointerEvent<SVGCircleElement | SVGGElement>, nodeIds: string[]) => {
      const nodeDetails = nodeIds.map(id => {
        ...
        const location = [nodeData.geo.city, nodeData.geo.country, nodeData.geo.continent]
          .filter(Boolean)
          .join(', ');
            ...
            <>
              ...
              <div className="font-bold text-accent border-b border-subtle pb-1 mb-1">
                ...
                <span className="text-primary ml-1">
                  ({visibleNodes.length} {visibleNodes.length === 1 ? 'node' : 'nodes'})
                </span>
              </div>
                    ...
                    {visibleNodes.map(node => (
                      <tr key={node.id} className="border-b border-subtle/30 last:border-0">
                        ...
                        <td className="py-1 pr-1 text-tertiary whitespace-nowrap text-right align-top">
                          {node.source}{' '}
                          <span className="text-secondary">{(node.time / 1000).toFixed(1)}s</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      <div className="h-full relative">
        ...
        <Card className="absolute bottom-1 left-1 px-1.5 py-0.5">
          <p className="text-[9px] font-mono text-tertiary">
            Data from nodes contributing to Xatu • Not representative of actual Ethereum network
            distribution
          </p>
        </Card>
      </div>
        ...
        {markers.map(
          ({
            ...
            const isFirstLocation = nodeIds.some(nodeId => {
              const nodeEvents = filteredBlockEvents.filter(
                e => e.node === nodeId && e.time <= currentTime && e.time > 0,
              );

              if (nodeEvents.length === 0) return false;

              // Get source of first event for this node
              const firstNodeEvent = nodeEvents.reduce(
                (min, e) => (e.time < min.time ? e : min),
                nodeEvents[0],
              );
              const source = firstNodeEvent.source;

              // Get all events for this source
              const allEventsForSource = filteredBlockEvents.filter(
                be => be.source === source && be.time <= currentTime && be.time > 0,
              );
            ...
            const glowColor =
              dominantSource === 'api' ? 'rgba(96, 165, 250, 0.3)' : 'rgba(168, 85, 247, 0.3)';
