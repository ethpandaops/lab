    ...
    if (currentSlotData && currentSlotData.relayBids) {
      ...
      Object.entries(currentSlotData.relayBids).forEach(([relayName, relayData]: [string, any]) => {
        ...
        const negativeBids = relayData.bids.filter(bid => {
          ...
          const bidTime = bid.time || bid.slotTime || bid.slot_time || 
                         bid.time_bucket || bid.timeBucket || // some use time buckets
                         bid.slot_time;
          ...
          try {
            ...
            if (valueInEth > 0) {
              processedBids.push({
                value: valueInEth,
                relayName,
                time: bid.time || bid.slotTime || bid.slot_time || 
                      bid.time_bucket || bid.timeBucket || -10000, // Use whichever time field is available
                builderName: bid.builderName || (bid.builderPubkey ? 
        ...
        <div className="flex justify-between items-start mb-2">
          <div className="flex items-center">
            <div className="w-8 h-8 bg-slate-800 rounded-lg flex items-center justify-center mr-2 relative overflow-hidden">
              ...
              <svg className="w-4 h-4 text-cyan-400 relative z-10" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
