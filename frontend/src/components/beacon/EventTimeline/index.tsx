...
const EventItem = memo(function EventItem({
  ...
  return (
    <div
      className={clsx('rounded bg-surface/40 text-[9px] md:text-[10px]', !isActive && 'opacity-50')}
    >
      <div className="px-1.5 md:px-2 py-0.5 md:py-1 flex items-center gap-1 md:gap-1.5 truncate">
        ...
        <span className="text-tertiary ml-auto shrink-0">{eventTime.toFixed(1)}s</span>
      </div>
    </div>
  );
});
          ...
          <label className="flex items-start gap-3 p-2 hover:bg-surface/50 rounded-lg cursor-pointer transition-colors group">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-primary font-medium group-hover:text-cyan-400 transition-colors">
                  Blocks (API)
                </span>
              </div>
          ...
          <label className="flex items-start gap-3 p-2 hover:bg-surface/50 rounded-lg cursor-pointer transition-colors group">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-primary font-medium group-hover:text-purple-400 transition-colors">
                  Blocks (P2P)
                </span>
              </div>
              <div className="text-tertiary text-xs mt-0.5">
                Blocks seen on the p2p network by sentry nodes
              </div>
            </div>
          </label>
          ...
          <label className="flex items-start gap-3 p-2 hover:bg-surface/50 rounded-lg cursor-pointer transition-colors group">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-primary font-medium group-hover:text-blue-400 transition-colors">
                  Blobs (API)
                </span>
              </div>
          ...
          <label className="flex items-start gap-3 p-2 hover:bg-surface/50 rounded-lg cursor-pointer transition-colors group">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-primary font-medium group-hover:text-fuchsia-400 transition-colors">
                  Blobs (P2P)
                </span>
              </div>
          ...
          <label className="flex items-start gap-3 p-2 hover:bg-surface/50 rounded-lg cursor-pointer transition-colors group">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-primary font-medium group-hover:text-success transition-colors">
                  Attestations
                </span>
              </div>
...
export const EventTimeline = memo(function EventTimeline({
  ...
  const sortedEvents = useMemo(() => {
    return events
      .filter(event => {
        ...
        switch (event.type) {
          case 'Block Seen (API)':
            return filters.blocksApi;
          case 'Block Seen (P2P)':
            return filters.blocksP2p;
          case 'Blob Seen (API)':
            return filters.blobsApi;
          case 'Blob Seen (P2P)':
            return filters.blobsP2p;
          case 'Attestation':
            return filters.attestations;
          default:
            return true;
        }
      })
  ...
  return (
    ...
    >
      ...
      <div className="flex-none border-b border-subtle">
        ...
        <div className="flex items-center justify-between p-1.5 md:p-3 pb-1 md:pb-2">
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div className="text-[10px] font-mono text-tertiary hidden md:block">
              {filters.node ? (
                <span>
                  Events from {formatNodeName(filters.node).user}'s{' '}
                  {formatNodeName(filters.node).node}
                </span>
              ) : filters.username ? (
                <span>
                  Events from {filters.username}'s nodes ({nodeCount})
                </span>
              ) : (
        ...
        <div className="px-2 md:px-3 pb-1.5 md:pb-3 flex items-center justify-between">
          <div className="flex items-center gap-1 md:gap-1.5">
            ...
            >
              {isPlaying ? (
                <FaPause className="w-2 h-2 md:w-3 md:h-3" />
              ) : (
                <FaPlay className="w-2 h-2 md:w-3 md:h-3 ml-0.5" />
              )}
            </button>
            <button
              onClick={onNextSlot}
        ...
        <div className="px-2 md:px-3 pb-1.5 md:pb-3">
          ...
          >
            ...
            <div className="absolute inset-0 flex">
              ...
              <div className="w-1/3 flex items-center justify-center">
                <span className="text-[11px] font-mono font-medium text-yellow-400">
                  Aggregation
                </span>
              </div>
            </div>
      ...
      <div
        ref={timelineRef}
        className="flex-1 min-h-0 overflow-y-auto scrollbar-hide w-full overscroll-contain"
      >
        <div className="p-0.5 md:p-2 space-y-0.5 pb-16">
          {loading ? (
            <div className="space-y-1.5">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="rounded bg-surface/40 p-2">
