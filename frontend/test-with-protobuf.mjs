...
} from './src/api/gen/backend/pkg/api/v1/proto/public_pb.js';
import { transformToBeaconSlotData } from './src/utils/slotDataTransformer.js';
import fs from 'fs';
...
async function fetchAndTransformWithProtobuf() {
  ...
  try {
    ...
    results.forEach((result, i) => {
      const names = [
        'Block', 'Block Timing', 'Blob Timing', 'Blob Total',
        'Attestation Timing', 'Attestation Correctness',
        'MEV Block', 'MEV Relay', 'MEV Builder'
      ];
    ...
    console.log(`Block seen timings: ${Object.keys(transformedData.timings?.blockSeen || {}).length}`);
    console.log(`Block P2P timings: ${Object.keys(transformedData.timings?.blockFirstSeenP2p || {}).length}`);
    console.log(`Blob seen timings: ${Object.keys(transformedData.timings?.blobSeen || {}).length}`);
    console.log(`Blob P2P timings: ${Object.keys(transformedData.timings?.blobFirstSeenP2p || {}).length}`);
