import { forwardRef, cloneElement, isValidElement, useId, type JSX } from 'react';
...
export const InputGroup = forwardRef<HTMLInputElement, InputGroupProps>(
  (
    ...
    const describedBy = [ariaDescribedBy, cornerHint ? `${id}-optional` : undefined, helpTextId, errorId]
      .filter(Boolean)
      .join(' ');
    ...
    const getInputClasses = (): string => {
      // Inline addons - wrapper has the outline, input has minimal styling (no grid needed, using flex)
      if (inlineLeadingAddon || inlineTrailingAddon) {
        const paddingLeft = inlineLeadingAddon ? 'pl-1' : 'pl-3';
        return `block min-w-0 grow py-1.5 pr-3 ${paddingLeft} text-base text-foreground placeholder:text-gray-400 focus:outline-none sm:text-sm/6 dark:text-white dark:placeholder:text-gray-500 ${className}`;
      }
      ...
      if (variant === 'inset') {
        return `${base} text-foreground placeholder:text-gray-400 focus:outline-none dark:bg-transparent dark:text-white dark:placeholder:text-gray-500`;
      }

      if (variant === 'overlapping') {
        return `${base} rounded-md bg-white px-3 py-1.5 text-foreground outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-primary dark:bg-background dark:text-white dark:outline-border dark:placeholder:text-gray-500 dark:focus:outline-primary ${
          disabled
            ? 'cursor-not-allowed bg-gray-50 text-gray-500 outline-gray-200 dark:bg-white/10 dark:text-gray-500 dark:outline-white/5'
            : ''
        } ${className}`;
      }

      if (variant === 'pill') {
        return `${base} rounded-full bg-white px-4 py-1.5 text-foreground outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-primary dark:bg-white/5 dark:text-white dark:outline-white/10 dark:placeholder:text-gray-500 dark:focus:outline-primary ${
          disabled
            ? 'cursor-not-allowed bg-gray-50 text-gray-500 outline-gray-200 dark:bg-white/10 dark:text-gray-500 dark:outline-white/5'
            : ''
        } ${className}`;
      ...
      if (variant === 'bottom-border') {
        return `${base} bg-gray-50 px-3 py-1.5 text-foreground placeholder:text-gray-500 focus:outline-none dark:bg-white/5 dark:text-white dark:placeholder:text-gray-500 ${className}`;
      }

      // Default variant
      const errorClasses = hasError
        ? 'text-red-900 outline-1 -outline-offset-1 outline-red-300 placeholder:text-red-300 focus:outline-2 focus:-outline-offset-2 focus:outline-danger dark:text-red-400 dark:outline-red-500/50 dark:placeholder:text-red-400/70 dark:focus:outline-red-400'
        : 'text-foreground outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-primary dark:text-white dark:outline-gray-700 dark:placeholder:text-gray-500 dark:focus:outline-primary';
      ...
      return `${base} ${roundingClasses} ${marginClasses} ${paddingClasses} ${errorClasses} ${disabledClasses} ${className}`;
    };
    ...
    return (
      <div className={wrapperClassName}>
          ...
          <>
            {(label || cornerHint) && (
              <div className="flex justify-between">
                ...
                {cornerHint && (
                  <span id={`${id}-optional`} className="text-sm/6 text-gray-500 dark:text-gray-400">
                    {cornerHint}
                  </span>
                )}
              </div>
        ...
        {/* Error message */}
        {error && (
          <p id={errorId} className="mt-2 text-sm text-danger dark:text-red-400">
            {error}
          </p>
        )}
      </div>
